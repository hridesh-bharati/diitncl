  ...
  return (
    <div className="container-fluid dashboard-wrapper">
      ...
      <div className="row g-3 mb-4">
        {STAT_CARDS.map((card, i) => (
          <div className="col-6 col-lg-3" key={i}>
            <div
              className="stat-card p-3 mb-3 rounded-4 shadow-sm position-relative text-white"
              style={{ background: card.bg, cursor: 'pointer', overflow: 'hidden' }}
            ...
            >
              ...
              <div className="position-relative" style={{ zIndex: 2 }}>
                <div className="d-flex align-items-center mb-1">
                  ...
                  <h2 className="fw-bold mb-0">{card.val}</h2>
                </div>
          ...
          <table className="table table-hover mb-0">
            ...
            <tbody>
              {students.map(s => {
                ...
                if (s.regNo?.startsWith("DIIT124")) { branch = "Main"; branchClass = "bg-primary"; }
                else if (s.regNo?.startsWith("DIIT125")) { branch = "East"; branchClass = "bg-success"; }

                return (
                  <tr key={s.id} onClick={() => navigate(`/admin/students/${s.id}`)} style={{ cursor: 'pointer' }}>
                    <td className="ps-3 align-middle border-0">
                      <div className="d-flex align-items-center gap-2">
                        <div className="avatar-circle" style={{ width: '35px', height: '35px', flexShrink: 0 }}>
                          {s.photoUrl ? 
                            <img src={s.photoUrl} alt="" className="w-100 h-100 rounded-circle border" style={{ objectFit: 'cover' }} /> 
                            : <div className="w-100 h-100 rounded-circle bg-light d-flex align-items-center justify-content-center fw-bold">{s.name?.charAt(0)}</div>
                          }
                        </div>
                        <div className="d-flex flex-column">
                          <div className="d-flex align-items-center gap-1">
                            <span className="fw-bold text-dark" style={{ fontSize: '13px' }}>{s.name}</span>
                            <span className={`badge ${branchClass} fw-bold py-1 px-2`} style={{ fontSize: '9px', borderRadius: '4px' }}>
                              {branch}
                            </span>
                          </div>
      ...
      <div className="mt-4">
        ...
        <div className="row g-3 pb-5 mb-5 mb-lg-0">
          {queries.map(q => (
            <div className="col-6 col-md-3" key={q.id}>
              <div className="enquiry-card p-3 bg-white shadow-sm rounded-3 border h-100" onClick={() => navigate("/admin/clients-contacts")} style={{ cursor: 'pointer' }}>

...
import { 
  PersonCircle, PeopleFill, Check2Circle, EnvelopeFill, 
  Images, ArrowRight, Telephone, LightningFill 
  ...
  useEffect(() => {
    const unsubStudents = onSnapshot(query(collection(db, "admissions"), orderBy("createdAt", "desc")), (snap) => {
      ...
      setStats(prev => ({ 
        ...
        admittedToday: data.filter(s => s.createdAt?.toDate?.().toDateString() === today).length 
      }));
    ...
    return () => { unsubStudents(); unsubQueries(); unsubGallery(); };
  }, []);

  const STAT_CARDS = useMemo(() => ([
    { label: "Students", value: stats.totalStudents, icon: <PeopleFill />, bg: "linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)", link: "students" },
    { label: "New Today - Adm", value: stats.admittedToday, icon: <LightningFill />, bg: "linear-gradient(135deg, #10b981 0%, #3b82f6 100%)", link: "studends" },
    { label: "Enquiries", value: stats.pendingQueries, icon: <EnvelopeFill />, bg: "linear-gradient(135deg, #f59e0b 0%, #ef4444 100%)", link: "/admin/clients-contacts" },
    { label: "Gallery", value: loading.gallery ? "..." : galleryCount, icon: <Images />, bg: "linear-gradient(135deg, #ec4899 0%, #f43f5e 100%)", link: "/gallery" },
  ]), [stats, galleryCount, loading.gallery]);

  return (
    <Container fluid className="py-3 py-lg-5 px-3 px-lg-5" style={{ backgroundColor: "#f8fafc", minHeight: "100vh", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      
      {/* --- HEADER --- */}
      <header className="mb-4 d-flex justify-content-between align-items-center">
        <div>
          <span className="text-primary fw-bold small text-uppercase ls-1">Admin Console</span>
          <h2 className="fw-800 text-dark m-0 d-none d-md-block" style={{ letterSpacing: "-1.5px" }}>Institute Insights</h2>
        ...
        <div className="glass-card p-2 px-3 rounded-4 shadow-sm border border-white text-end">
          <div className="fw-bold text-dark small">{new Date().toLocaleDateString('en-US', { day: 'numeric', month: 'short' })}</div>
          <small className="text-muted d-block" style={{fontSize: '10px'}}>System Active</small>
        </div>
      </header>

      {/* --- STATS GRID WITH WATERMARK --- */}
            ...
            <Link to={card.link} className="text-decoration-none">
              <div className="stat-card p-3 p-lg-4 h-100" style={{ background: card.bg }}>
                ...
                <div className="d-flex ms-2">
                  ...
                  <div className="text-white opacity-75 small fw-600 mt-1">{card.label}</div>
                </div>
              </div>
            </Link>
          </Col>
        ...
        <Col lg={8}>
          <Card className="main-card border-0 shadow-sm h-100 overflow-hidden">
            <Card.Header className="bg-white border-0 p-4 d-flex justify-content-between align-items-center">
              ...
              <Link to="admitted-student-list" className="btn-pill-sm">View All</Link>
            </Card.Header>
                      ...
                      <td>
                        <div className="d-flex align-items-center">
                          <div className="avatar-box me-3">
                            {s.photoUrl ? <img src={s.photoUrl} alt="" /> : <PersonCircle size={30} className="text-secondary opacity-25" />}
                          </div>
                          <div className="d-flex flex-column">
                            <span className="fw-700 text-dark" style={{fontSize: '14px'}}>{s.name}</span>
                            <span className="d-md-none " style={{fontSize: '11px'}}>{s.course}</span>
                          </div>
                        </div>
                      </td>
                      <td className="d-none d-md-table-cell">
                        <Badge pill className="badge-soft-indigo text-white">{s.course}</Badge>
                      </td>
          ...
          <Card className="main-card border-0 shadow-sm bg-white">
            ...
            <Card.Body className="p-4 pt-0">
              <div className="query-list">
                {loading.queries ? (
                  <div className="text-center py-4"><Spinner animation="grow" variant="warning" size="sm" /></div>
                  ...
                  <div key={q.id} className="query-card p-3 mb-3">
                    <div className="d-flex justify-content-between align-items-start mb-1">
                      ...
                      <span className="time-tag">{q.timestamp?.toDate?.().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <p className="text-muted small text-truncate mb-3" style={{maxWidth: '200px'}}>
                      {q.title || "Subject not specified"}
                    </p>
                    <div className="d-flex justify-content-between align-items-center">
                      <a href={`tel:${q.mobile}`} className="call-link">
                        <Telephone size={12} className="me-2"/> Call Student
                      </a>
                      <Link to="/admin/clients-contacts" className="text-primary"><ArrowRight size={16}/></Link>
                    </div>
                  </div>
                ))}
              </div>

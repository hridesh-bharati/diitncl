...
export default function AdminCompletedExams() {
  ...
  // 2. URL se examId nikalne ka logic
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snap) => {
      const res = await Promise.all(snap.docs.map(async (d) => {
        const data = d.data();
        const [sS, eS] = await Promise.all([getDoc(doc(db, "admissions", data.studentId)), getDoc(doc(db, "exams", data.examId))]);
        return { id: d.id, ...data, student: sS.data() || {}, exam: eS.data() || { totalMarks: 70 } };
      }));
  ...
  const list = useMemo(() => {
    return completed.filter(i => {
      // 3. Date filter + ExamId filter dono lagaye
  ...
  }, [completed, filterDate, filterExamId]); // 4. Dependency mein filterExamId add kiya

  if (loading) return <div className="vh-100 d-flex justify-content-center align-items-center"><div className="spinner-border text-primary border-3" /></div>;
  ...
  return (
    <div className="container-fluid p-2 bg-light min-vh-100">
      <div className="d-flex justify-content-between align-items-center bg-white p-2 border mb-3 shadow-sm rounded">
        <div>
          {/* Heading change ki taaki pata chale filter on hai */}
          <h6 className="m-0 fw-bold small uppercase">
            {filterExamId ? "Filtered Results" : "Synced Results"} ({list.length})
          </h6>
          {filterExamId && <small className="text-primary fw-bold" style={{ cursor: 'pointer', fontSize: '9px' }} onClick={() => navigate('/admin/exams/completed')}>Show All Results</small>}
        </div>
          ...
          {list.length > 0 ? (
            list.map(item => (
              <div key={item.id} className="col-12 col-md-6 col-lg-4">
                {/* Aapka Card wala pura code yahan rahega */}
                <div className={`card border-0 shadow-sm rounded-0 h-100 ${item.isAdminViewed ? 'opacity-50 border-start border-4 border-secondary' : 'border-start border-4 border-primary'}`}>
                  <div className={`card border-0 shadow-sm rounded-0 ${item.isAdminViewed ? 'opacity-50 border-start border-4 border-secondary' : 'border-start border-4 border-primary'}`}>
                    ...
                    <div className="card-body p-2">
                      ...
                      <div className="d-flex justify-content-between mb-2">
                        ...
                        <div className="d-flex align-items-center gap-2 overflow-hidden">
                          ...
                          <img src={item.student?.photoUrl || `https://ui-avatars.com/api/?name=${item.student?.name}`} className="rounded border" width="35" height="35" />

                          <div className="text-truncate">

                            <div className={`fw-bold small text-uppercase text-truncate ${item.isAdminViewed ? 'text-decoration-line-through' : ''}`} style={{ fontSize: '11px' }}>{item.student?.name}</div>
                      ...
                      <div className="d-flex justify-content-between align-items-center bg-light p-2 border-top rounded">
                        ...
                        <div className="d-flex gap-1">
                          ...
                          <button className={`btn btn-sm border p-1 px-2 ${item.isAdminViewed ? 'btn-success text-white' : 'btn-white text-muted'}`} onClick={() => updateDoc(doc(db, "studentExams", item.id), { isAdminViewed: !item.isAdminViewed })}>
                          ...
                          <button className="btn btn-sm btn-white border p-1 px-2 text-danger" onClick={() => { if (window.confirm('Delete?')) deleteDoc(doc(db, "studentExams", item.id)) }}><i className="bi bi-trash" /></button>
            ...
            // 🔥 AGAR LIST KHALI HAI TO YE DIKHEGA

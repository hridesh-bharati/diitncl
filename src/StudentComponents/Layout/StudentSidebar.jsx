...
export default function StudentSidebar({ open, setOpen }) {
  ...
  const menuItems = [
    ...
    { to: "/student/exams", icon: "bi-journal-check", label: "Main Exams" },

    // 🔥 NEW PRACTICE TEST
    { to: "/student/practice-tests", icon: "bi-ui-checks-grid", label: "Practice Test" },

    // 🔥 NEW MY RESULTS
    // { to: "/student/practice-results", icon: "bi-bar-chart-line", label: "My Results" },
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div
        className={`d-none d-lg-flex flex-column vh-100 border-end bg-white shadow-sm desktop-sidebar ${
          collapsed ? "collapsed" : ""
        }`}
        style={{
      ...
      >
        ...
        >
          {!collapsed && (
            <h5 className="fw-bold mb-0 text-primary animate__animated animate__fadeIn">
              DRISHTEE LMS
            </h5>
          )}
          ...
          >
            <i
              className={`bi ${
                collapsed ? "bi-chevron-right" : "bi-list"
              } fs-5`}
            ></i>
          </button>
        </div>
        ...
        <div className="p-3 border-top">
          <button
            className={`btn btn-outline-danger w-100 border-0 d-flex align-items-center ${
      ...
      >
        ...
        <div className="offcanvas-body p-3 bg-light">
          ...
          <div className="mt-5 p-2 border-top pt-4 text-center">
            <button
              className="btn btn-danger w-100 rounded-pill py-2 shadow-sm fw-bold"
              onClick={() => auth.signOut()}
            >

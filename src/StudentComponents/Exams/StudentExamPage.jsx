...
export default function StudentExamPage() {
  ...
  useEffect(() => {
    const fetchInitialInfo = async () => {
      ...
      try {
        ...
        // Initializing Answers: Programming questions get their starter code
  ...
  useEffect(() => {
    ...
    const handleVisibility = () => { if (document.hidden) handleFinalSubmit(true); };
    document.addEventListener("visibilitychange", handleVisibility);
    const timer = setInterval(() => setTimeLeft(t => t > 0 ? t - 1 : 0), 1000);
    if (timeLeft === 0 && examStarted) handleFinalSubmit(true);
    return () => {
  ...
  <div className="container py-3" style={{ maxWidth: "850px" }}>
    ...
    <div className="card border-0 shadow-sm rounded-0 mb-3">
      <div className="card-body p-3 p-md-4">
        ...
        {/* QUESTION */}
        <h6 className="fw-bold mb-3 lh-base">
          {q?.question}
        </h6>
        ...
        {q?.type === "mcq" ? (
          <div className="d-grid gap-2">
            ...
            {["A", "B", "C", "D"].map((opt) => (
              <button
                key={opt}
                className={`btn text-start rounded-0 border ${
                  ans[q.id] === opt
                    ? "btn-primary"
              ...
              >
                <span className="fw-semibold me-2">{opt}.</span>
                {q[`option${opt}`]}
              </button>
            ))}
          ...
          <div className="border rounded-0">
            ...
            <div className="bg-light px-2 py-1 border-bottom small text-muted fw-semibold">
            ...
            <Editor
              ...
              highlight={(code) =>
                highlight(
                  code,
                  languages[q.language] || languages.js
                )
              }
              padding={12}
              style={{

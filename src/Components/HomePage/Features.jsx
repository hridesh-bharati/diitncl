    ...
    <section className="container py-5">
      <div className="row align-items-center g-4">
        ...
        <div className="col-lg-7"><div className="row g-3">
          {DATA.map((item, i) => (
            <div key={i} className="col-6"><div className="f-card p-4 rounded-4 shadow-sm text-center bg-white border border-light h-100">
      ...
      <style>{`.f-card{transition:.3s;cursor:pointer}.f-card:hover{transform:translateY(-8px);border-color:#0d6efd!important}.i-box{width:55px;height:55px;border-radius:15px}.dash{height:3px;width:25px;border-radius:2px}.extra-small{font-size:11px}`}</style>
    </section>
  );
}
...
export default function Verification() {
  ...
  const handleSearch = (admissions) => {
    setError("");
    if (!regNo || !studentName) return setError("Fill both fields");

    const match = admissions.find(s =>
      s.regNo?.toUpperCase() === regNo.trim().toUpperCase() && clean(s.name) === clean(studentName)
    );
          ...
          <div className="card shadow-lg border-0 rounded-4 overflow-hidden" style={{ maxWidth: "380px", width: "100%" }}>
            ...
            <div className="card-body p-4 text-center">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleSearch(admissions); }}>
                <input className="form-control mb-3 py-2 border-2 shadow-none" placeholder="REGISTRATION NO" value={regNo} onChange={e => setRegNo(e.target.value.trim().toUpperCase())} />
                <input className="form-control mb-4 py-2 border-2 shadow-none" placeholder="STUDENT FULL NAME" value={studentName} onChange={e => setStudentName(e.target.value)} />
                <div className="mb-4 border p-2 rounded bg-light"><Captcha onVerify={setCaptchaVerified} /></div>
                <button className="btn btn-primary w-100 py-2 fw-bold shadow-sm" disabled={!captchaVerified} style={{ background: "#4F46E5" }}>VERIFY NOW</button>
              </form>

const a="/api/send-mail",s="/api/send-push",l=async(t,e,o,i="/student/dashboard")=>{if(!t?.pushSubscription)return!1;try{return(await(await fetch(s,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({subscription:JSON.parse(t.pushSubscription),title:e,body:o,url:`https://www.drishteeindia.com${i}`})})).json().catch(()=>({success:!1}))).success}catch{return!1}},c=async(t,e,o)=>{try{return(await(await fetch(a,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:t,subject:e,html:o})})).json().catch(()=>({success:!1}))).success}catch{return!1}},f=t=>`
...
`,y=(t,e)=>{const o=e.duration||1,i=`${o} ${o<=1?"Hour":"Hours"}`,r=e.date||"As per Schedule";let d=e.startTime;try{e.startTime&&!e.startTime.includes("M")&&(d=new Date(`2000-01-01T${e.startTime}`).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",hour12:!0}))}catch{d=e.startTime}const n=e.endTime||"As per Schedule";return`
  <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 650px; margin: 20px auto; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.08); background-color: #fff;">
...
`;export{l as a,x as b,f as c,h as d,b as e,g as f,y as g,u as o,c as s};

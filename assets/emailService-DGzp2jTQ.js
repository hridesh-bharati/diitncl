const i="https://diitnclserver.onrender.com",a=async(e,t,o)=>{try{const r=await fetch(`${i}/api/send-mail`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:e,subject:t,html:o})});if(!r.ok){const n=await r.text();return!1}return(await r.json()).success}catch{return!1}},l=e=>`
...
`,f=(e,t)=>{const o=t.duration||1,r=`${o} ${o<=1?"Hour":"Hours"}`;return`
  <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 650px; margin: 20px auto; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.08); background-color: #fff;">
